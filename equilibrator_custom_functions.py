...
def create_pathway(filename,cs,pc,absolute_flux = Q_(1.0, 'M/s'),custom_dGs = True,x = True):
	(reactions, fluxes,std_dGs) = custom_parse_reactions(filename,cs,absolute_flux = absolute_flux,custom_dGs = custom_dGs)
	...
	if x == True:
		pathway.dg_confidence = 0.95  #* confidence
	else:
...
def custom_parse_reactions(filename,cs,absolute_flux = Q_(1.0, 'M/s'),custom_dGs = True):
	with open(filename + '.csv','r') as csv_file:
		df = pd.read_csv(csv_file)
		csv_file.close()
	...
	for i in range(len(rxns)):
		...
		args = rxns[i].split()
		for arg in args:
			ANumber = all(map(str.isdigit, arg))
...
def MDF_analysis(pathway_file_name, cmpd_settings, physiological_context, custom_dGs = True, print_results = True, y = True):
	#compound = cmpd_settings.acr2cmpd_dict.get("AcCoA")
	
	if y == True:
		pathway = create_pathway(pathway_file_name,cmpd_settings,physiological_context,custom_dGs=custom_dGs, x = True)
	else:
		pathway = create_pathway(pathway_file_name,cmpd_settings,physiological_context,custom_dGs=custom_dGs, x = y)
...
def MDF_CVA(MDF, pathway_file_name,cmpd_settings, physiological_context, compounds = None, custom_dGs = True, print_results = True):
	if type(MDF) is Q_:
		MDF = MDF.m_as('kJ/mol')
	MDF_overRT = MDF/RT.magnitude
	pathway = create_pathway(pathway_file_name,cmpd_settings,physiological_context,custom_dGs=custom_dGs)
	if compounds is None:
		compounds = pathway.compound_names.copy()
	...
	if print_results:
		...
		ax.set_title('Concentration Variability Analysis of' + pathway_file_name + 'for MDF = ' + MDF + ' kJ/mol', fontsize=12)
		plt.tight_layout()
	return results
...
def concentration_variability_analysis(MDF_overRT, cmpd_acr, pathway):
	...
	compound_idx = pathway.compound_names.index(cmpd_acr)
	cmpd_names_except = pathway.compound_names.copy()
	cmpd_names_except.remove(cmpd_acr)
	reaction_names = [rxn.rid for rxn in pathway.reactions]

	thermo_model = PathwayThermoModel(pathway, pathway.stdev_factor)
	inds = np.nonzero(np.diag(thermo_model.I_dir))[0].tolist()
	#x is the vector of (y | log-conc | B)
	if thermo_model.dg_sigma_over_rt is None:
		A11 = np.zeros((len(inds), thermo_model.Nr))
	else:
		A11 = thermo_model.I_dir[inds] @ thermo_model.dg_sigma_over_rt
	A12 = thermo_model.I_dir[inds] @ thermo_model.pathway.S.T
	A13 = np.ones((len(inds), 1))
	# covariance var ub and lb
	A21 = np.eye(thermo_model.Nr)
	A22 = np.zeros((thermo_model.Nr, thermo_model.Nc))
	A23 = np.zeros((thermo_model.Nr, 1))
	# log conc ub and lb - remove constraints for tested metabolite
	A31 = np.zeros((thermo_model.Nc-1, thermo_model.Nr))
	A32 = np.eye(thermo_model.Nc)
	A32 = np.delete(A32, compound_idx, 0)
	A33 = np.zeros((thermo_model.Nc-1, 1))

	# upper bound values
	b1 = -thermo_model.I_dir[inds] @ thermo_model.standard_dg_primes_over_rt
	b2 = np.ones(thermo_model.Nr) * thermo_model.stdev_factor
	A = np.vstack(
		[
	...
	b = np.hstack(
		[b1, b2, b2, np.delete(thermo_model.pathway.ln_conc_ub, compound_idx, 0), -np.delete(thermo_model.pathway.ln_conc_lb, compound_idx, 0)]
	)
	if thermo_model.r_bounds:
		for i, r_ub in enumerate(thermo_model.r_bounds):
			if r_ub is not None:
				A[i, -1] = 0.0
				b[i] += float(r_ub)
	...
	lnC = [Variable("lnC_" + c_name) for c_name in pathway.compound_names]
	B = Variable("mdf")
	x = y + lnC + [B]
	...
	cnstr_names = (
		...
		+ ["covariance_var_ub_" + r_name for r_name in reaction_names]
		+ ["covariance_var_lb_" + r_name for r_name in reaction_names]
...
def ECM_analysis(pathway_file_name, cmpd_settings, physiological_context,abs_flux,custom_dGs=False):
	cs = cmpd_settings
...
def ECM_plot(model,ln_conc,ax,fontsize_ylabel = 12):
	...
	ax.set_ylabel("enzyme demand",fontsize = fontsize_ylabel)
