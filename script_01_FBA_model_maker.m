...
model=addReaction(model,'R5PK','Ri5P + ATP -> PRibPP + AMP');
model=addReaction(model,'RiBPK','PRibPP + ADP -> RiBP + ATP');
...
model=addReaction(model,'ADPPFK','FBP + AMP <=> F6P + ADP');
...
T0(rows_to_remove, :) = [];
writetable(T0, 'CO_to_PHB_model.csv');
